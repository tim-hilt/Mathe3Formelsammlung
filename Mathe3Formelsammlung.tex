...
\usepackage{scrlayer-scrpage, trfsigns, tikz}
...
\mybfcol{Standardrechteck im Laplacebereich}  
...
\begin{minipage}{.48\linewidth}
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[ymax=1.5,xlabel=\(t\),ylabel=\(r(t)\), width=\textwidth,]
        \addplot[very thick, maincolor, no markers] coordinates{
          (-1,0)
          (0,0)
          (0,1)
          (1,1)
...
\mybfcol{Eingangssignal, Ausgangssignal, Impulsantwort}

Eine inhomogene Differential- oder Differenzengleichung mit Stöfunktion \(r(t) \neq 0\) besitzt das \mybfcol{Eingangssignal \(r(t)\)}. Sie besitzt die Lösung \(x(t)\) bzw. \(f_k\).\\
...
Die Ausgangsfunktion entspricht demnach der \mytextcol{Faltung der Störfunktion \(r(t)\) mit der Impulsantwort \(g(k)/g_lk\)}.

\LTI{\(r(t)\)}{\(g(k)/g_k\)}{\(x(t)/f_k\)}
...
\mytextcol{Quantile} berechnen einen bestimmten Prozentsatz der Fläche unter der Verteilungsfunktion einer normalverteilten Zufallsvariable.
...
Der \mytextcol{Zufallsstreubereich} (ZSB) Ist ein Intervall, das zwei Quantile berechnet. ZSBs können nach unten, nach oben oder zweiseitig beschränkt sein. Der Zufallsstreubereich kann für ein gegebenes \(\mu\) oder für den arithmetischen Mittelwert \(\overline{X}\) eines gegebenen Datensatzes berechnet werden.
...
Auch hier kann der \mybfcol{Taschenrechner} eingesetzt werden (InvNormal). Hier sind die Werte \textbf{Area, $\mu$ und $\sigma$} verlangt. $\mu$ und $\sigma$ sind meist in der Aufgabenstellung gegeben, für Area muss:
...
Ist ein \mytextcol{Datensatz mit \(n\) Elementen} gegeben, so ändern sich die Formeln zu:
...
Nun muss selbstverständlich \mytextcol{für $\sigma$ in der InvNormal-Funktion \(\frac{\sigma}{\sqrt{n}}\)} eingegeben werden.

\newpage
\phantomsection%
\addtocounter{chapter}{1}
\addchaptertocentry{\arabic{chapter}}{Anhang}

\newpage
\phantomsection%
\addtocounter{section}{1}
\addsectiontocentry{A}{Tabellen aus Buch von Prof.\ Koch und Prof.\ Stämpfle}
